...
class HD_classifier:
	...
	def __init__(self, N_seats1,HD_dim, N_seats2, T, device, string, cuda = True):
		...
		#with the others.
		## NOT USED IN THE CURRENT VERSION OF THE WORK ##
		if string == 'sandwich':
			if cuda:
	...
	def learn_HD_proj_big_half(self,EEG,fs):
		...
		queeryVector = torch.cuda.ShortTensor(1,1).zero_()
		N_channels,learningEnd = EEG.size()
		# creation of the index to scan the training EEG segment
		index = np.arange(0,learningEnd,fs/2)
		for iStep in index:
			# creation of the H vector for each single window of 0.5 seconds contained in the training segment
			temp = self.learn_HD_proj(EEG[:,iStep:iStep+fs/2])
			# accumulation of all the H vectors as explained in the green rectange in Fig.2 (<40 seconds rectangle)
			queeryVector=torch.add(queeryVector,1,temp)
		# final green threhsold of fig.2.
...
def LBP_window_train(EEG,fs, totalNumberBP, label,T):
	''' Function used during training to take a bigger window of EEG and sum all the histograms
	of LBP frequency of 0.5 seconds windows inside.
	Parameters
		----------
	...
	i = 0
	for iStep in index:
		temp = LBP_extractor(EEG[:,iStep:iStep+fs/2],totalNumberBP,label,T)
		queeryVector[i,:]= temp.squeeze()
		i = i+1
	return queeryVector
...
def LGP_window_train(EEG,fs, totalNumberBP, label,T):
	''' Function used during training to take a bigger window of EEG and sum all the histograms
	...
	i = 0
	for iStep in index:
		temp = LGP_extractor(EEG[:,iStep:iStep+fs/2],totalNumberBP,label,T)
		queeryVector[i,:]= temp.squeeze()
		i = i+1
	return queeryVector
