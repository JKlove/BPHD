...
for Patient in PatientList:
	...
	for cross in range(crossValidationK[Patient-1]):
		for i in range(Seizure_train[Patient-1]):
			...
			temp1 = iEEGFunctions.LGP_window_train(EEGSez, fs, totalNumberBP, 1, T)
			if i == 0:
		...
		temp3=iEEGFunctions.LGP_window_train(EEGInterictal,fs, totalNumberBP, 0,T)
		...
		clf = MLPClassifier(solver='lbfgs', activation= 'tanh',max_iter =1000,alpha=1e-5, hidden_layer_sizes=(40,), random_state=1)
		...
		for i in range(seizureN[Patient-1]):
			...
			j = j+1
			for iStep in index:
				temp = iEEGFunctions.LGP_extractor(EEGtest[:,iStep:iStep+fs/2],totalNumberBP,2,T)
			...
			with open('/home/hyb/Desktop/HDC/HD/A/MLP_prediction/Pat'+\
				str(Patient)+'/' + 'MLPClassifier_Seizure'+str(j)+'Cross'+str(cross)+'.txt', 'wb') as f1:
